...
- V1 支援哪些 ActionKind、每一個會改寫什麼、以及由誰授權的**公開面向列舉**。
- **timelock-下限參考表**(§3) — 屬於 OptiVaults V1 的操作政策,不是通用模式的一部分。
- **跨 validator 授權流程**(§7) — MultisigGov 的 payload-hash 檢查與每個消費端 validator 的 payload 重算步驟之間的線路。
