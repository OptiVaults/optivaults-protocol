...
**Optional second adapter — SundaeSwap (config-gated).** Beyond `minswap_v2_adapter`, V1 can be deployed with a second SwapAdapter for SundaeSwap V3 + Stableswaps. It is two artefacts, not one:
...
Both are folded in only when the deploy-ceremony config carries a `sundaeswap` block (see `spec/swap-adapter.md` §8). Omitting the block leaves the ceremony byte-identical to the 22-artefact Minswap-only base; enabling it makes the count **24** (20 reference scripts). SundaeSwap serves V1's USDM leg (deep `USDCx/USDM` stableswap, lower like-asset slippage) plus a Minswap liveness backstop; the DJED leg stays on Minswap V2.
