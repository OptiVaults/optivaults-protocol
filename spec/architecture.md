...
| Operations | 25% | Platform-layer infrastructure (hosting for frontend / landing / API, Blockfrost platform queries, monitoring, domains, CDN); per-keeper infra is funded directly via the 40% keeper share at every Compound, not from this bucket | Governance-gated, 24h per-category cooldown |
...
- `max_staleness_ms` — per-feed age bound vs `tx.validity_range.lower_bound`. Stale feeds are dropped from aggregation. On-chain ceiling is 1 hour (`contracts/lib/vault/validation.ak::valid_asset_oracle_entry`); launch-recommended value is 10 minutes per entry, governance-tunable via `UpdateRegistry`.
...
Feed datum format (InlineDatum on the feed UTXO): `PriceSample { price_bps: Int, timestamp_ms: Int }`. Off-chain oracle operator aggregates native Charli3 + Orcfax feeds into this canonical format — the operator identity is an external trust boundary documented in `../docs/security-model.md`. V1 launches with `asset_oracles = []` and populates via governance `UpdateRegistry` as dual-feed coverage comes online per-asset (first DJED + USDM + ADA, then any additional Liqwid market adds).

---

## 4. Contract catalog

V1 ships with **17 logic validators + 4 NFT mint policies + `minswap_v2_adapter` + 2 SundaeSwap artefacts (`sundaeswap_adapter` + `sundaeswap_cancel_guard`) = 24 compiled artefacts total** (canonical count, used consistently across whitepaper §3.1, README size table, and `audit-scope.md` §6 scope statement). All are Aiken PlutusV3; compiled sizes recorded in `README.md`. Partitioning rationale (4 orthogonal seams: authorization-boundary, governance response-latency, bytecode-cost-center, size-fix) covered in §4.1 below.
...
| 4 | `vault_protocol` | Staking validator: DeployToProtocol (DEX, via SwapAdapter dispatch) | Compile-time parameterised by (keeper_stake_hash, governance_nft_policy, governance_nft_name); keeper-auth only; `verify_swap_via_adapter` (from `contracts/lib/vault/swap_adapter.ak`) consolidates adapter invocation + Tier 2 peg-floor + optional Tier 1 oracle bound; `publish` handler gated by ActDeregisterStake (A2) |
...
| 16 | `minswap_v2_adapter` | **SwapAdapter for Minswap V2 orders** (§B@launch=1). Staking validator invoked via zero-withdrawal by `vault_protocol.DeployToProtocol` + `vault_gov_emergency.AdminDeployNonDeposit`. Decodes Minswap V2 order datum (SwapExactIn / SwapMultiRouting) + verifies redeemer-committed min_receive + target_asset match on-chain datum. Unparameterised — hash whitelisted via Registry `swap_adapter_hashes`. Adding new DEX adapters post-launch goes through governance `UpdateRegistry` (14d timelock) — no V1 vault redeploy required. See `contracts/lib/vault/swap_adapter.ak` + `contracts/validators/minswap_v2_adapter.ak`. |

**Second DEX adapter — SundaeSwap.** Beyond `minswap_v2_adapter`, V1's second SwapAdapter targets SundaeSwap V3 + Stableswaps. It is two artefacts, not one:

- `sundaeswap_adapter` — a SwapAdapter conforming to the same interface as `minswap_v2_adapter` (catalog #16 / `spec/swap-adapter.md`). Structurally simpler than the Minswap adapter: SundaeSwap's order datum carries both swap assets explicitly (`Order::Swap { offer, min_received }`), so no LP-name rehash is needed. One adapter handles both SundaeSwap V3 (constant-product) and Stableswaps pools — they share an identical swap order datum.
- `sundaeswap_cancel_guard` — a staking validator that owns un-filled SundaeSwap orders. SundaeSwap authorises an order `Cancel` by the order datum's `owner` field; setting `owner` to this guard (rather than a plain keeper key) forces every cancel to satisfy `verify_cancel_value_conservation` — the net value leaving the vault must return to the vault address. This closes the path where a keeper could cancel a vault-funded order and redirect the refund. Minswap V2 needs no equivalent because its order datum pins the refund destination directly.

Both are bound at deploy-ceremony init — the ceremony config carries a `sundaeswap` block (see `spec/swap-adapter.md` §8) — so they are part of every V1 launch deploy, giving V1 its canonical count of **24** artefacts (20 reference scripts). SundaeSwap serves V1's USDM leg (deep `USDCx/USDM` stableswap, lower like-asset slippage) plus a Minswap liveness backstop; the DJED leg stays on Minswap V2.
...
V1's 17 logic validators (+ 4 NFT mint policies + `minswap_v2_adapter` + 2 SundaeSwap artefacts = 24 artefacts total) reflect partitioning along **four orthogonal seams**, each driven by the Plutus V3 16 KB reference-script ceiling:
...
- **OptiVaults mitigation**: 30% idle buffer as first-order liquidity insulation; `EmergencyWithdraw` governance write-off path for bad-debt scenarios; `AdminDeployNonDeposit` Minswap V2 recovery path independent of Liqwid; per-market `LiqwidPosition` isolation. Full analysis in `../docs/security-model.md`.
