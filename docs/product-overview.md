...
- **Audit funding stack with explicit Catalyst-pause disclosure.** Primary funding stack is (a) Cardano Project Catalyst grant ($30-50K) — **but Catalyst is currently paused / restructuring with no confirmed Round resumption timeline**; (b) audit-firm public-goods rate (-30-50% off the **$100-150K pre-discount base** price); (c) scope reduction from heritage internal audit (-$15-25K); (d) founder out-of-pocket. The **$50-150K engaged-amount range** in whitepaper §8.1 spans post-discount low-end (~$50K) to pre-discount full price (~$150K). **Audit timeline is set to Q2-Q3 2027** explicitly to accommodate Catalyst resumption probability + alternative-grant outreach (Cardano Foundation, Intersect, Aiken Foundation) + founder runway accumulation. **Worst-case founder out-of-pocket — all three mitigations underperform simultaneously**: $100-150K (full audit base price falls entirely on the founder). This is plausibly an automatic §4.1 sunset trigger if it lands, because paying it consumes the bulk of remaining founding-capital runway. Whitepaper §8.1 spells out the funding-stack disclosure + four contingency options (timeline extension / scope reduction / indefinite deferral / §0-framed sunset).
