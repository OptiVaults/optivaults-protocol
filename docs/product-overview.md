...
**V1 is pre-audit software under a 100K USDCx (≈$100K) total-deposits ceiling.** This is a ceiling, not a target — Phase 1 expected total TVL is actually **$5K-$25K** over the first 6-12 months. Read that as: we want a small number of pragmatic depositors to help us validate the system under real conditions, not a land-rush. External audit is scheduled for Q3 2026, after which the cap will be raised.
...
**Our mitigation**: multi-round internal audit completed before launch; external audit target Q3 2026; 100K USDCx cap limits maximum exposure during pre-audit era; `EmergencyWithdraw` governance path to freeze + recover if a bug is detected but not yet drained.
...
**Q: Is this audited?**
A: Internal audit has completed multiple rounds of adversarial validator-code review. External third-party audit is scheduled for Q3 2026. V1 operates under a 100K USDCx cap until external audit completes. See whitepaper §5.1 for methodology.
...
- **Audit funding has a contingency.** Primary funding source is a Catalyst grant ($30-50K) plus public-goods rate audit pricing (-30-50% off commercial $50-150K range) plus founder out-of-pocket ($20-40K cap). If Catalyst grant fails, the remaining sources still cover audit cost with founder out-of-pocket cap raised to ~$70K (single-failure-mode tolerated). Whitepaper §8.1 spells out the contingency math.
