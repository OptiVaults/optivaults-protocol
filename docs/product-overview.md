...
**V1 is a Cardano DeFi public-goods reference implementation, not a commercial product** — the 4.5% performance fee covers protocol operations and audit reserve; **there is no equity, no token, no investor distribution**. **Disclosure on the keeper share**: 40% of the performance fee (~1.8% of yield) goes to the keeper wallet as operational compensation. Because the founder runs the keeper at V1 launch, **USDCx does flow to the founder's keeper wallet** (~$108/year at the 100K cap; <$30/year in realistic Phase 1 sub-scenarios). This is **below the keeper's own infra cost** ($400-1000/year for infrastructure + monitoring per whitepaper §4.3), so the keeper position is a **net cost-centre, not a profit line**, for the founder — the loss is absorbed against founding-capital runway. Apache 2.0 license allows other teams to fork and specialize. Depositors should enter with a "contributing to a public good + being an early validator" mindset, not as purchasers of a commercial service (full positioning in whitepaper Executive Summary + §0 + §12 Disclosure).

**V1 is pre-audit software under a 100K USDCx (≈$100K) total-deposits ceiling.** This is a ceiling, not a target — Phase 1 expected total TVL is **$500-$25K** over the first 6-12 months (lower bound reflects permissionless contract + small-deposit reality; see whitepaper §4.1 + §8.2 for the three-scenario breakdown). Read that as: we want a small number of pragmatic depositors to help us validate the system under real conditions, not a land-rush. External audit is targeted for **Q2-Q3 2027** (reflecting Cardano Project Catalyst Round timing uncertainty — see whitepaper §8.1), after which the cap will be raised.

**Note on the current deployment (read carefully).** A separate **internal-verification-era contract iteration** (pre-V1) is already live on Cardano mainnet with **founder-gated frontend access** (deposit restricted to founder wallet at `vault.optivaults.app`; withdraw open to all internal-verification-era depositors). That deployment is **not** the V1 design described in this document — V1 is a fresh contract set that will undergo its own deploy ceremony per whitepaper §8.2 once launch-readiness conditions (§1.5) are met. Existing internal-verification-era depositors get a migration window at V1 launch; new depositors enter directly via the V1 contracts. See whitepaper §0 + §4.1 + §8.2 + `migration.md` for the distinction.
...
- **Compounds** yield — takes the interest Liqwid has paid, deducts our 4.5% performance fee, adds the rest back to the vault's total. **Cadence is TVL-tier-dependent**: at the post-audit 1,200+ USDCx tier under §2.6 Default cadence, Compound runs once per week (Saturday-anchored); at the realistic Phase 1 sub-scenario (a) $500–$5K under §2.6.1 Reference Implementation Mode, Compound runs quarterly to annually (only fires when accrued yield > $5 USDCx, to keep gas cost proportional to harvest); at sub-scenario (b) $5K–$25K, cadence tilts toward monthly–biweekly as TVL approaches $25K. The current operational mode is published on the OptiVaults dashboard. **Why this matters to you**: if you deposit during Phase 1 low TVL, your share price updates infrequently — this is by design (operational efficiency), not a fault. See whitepaper §2.6 + §2.6.1 for the full cadence policy.
...
OptiVaults V1 is a non-custodial stablecoin vault on Cardano that launches on mainnet under a **pre-audit 100K USDCx hard cap**. Whitepaper §0 spells out the full 4-phase model (Pre-Catalyst → Pre-Audit Mainnet → Audit + Cap-Lift Window → Operational); this overview uses a user-facing 2-phase simplification:

- **Phase 1 user-facing (pre-audit mainnet; whitepaper Phase 2 entry)**: V1 operates on mainnet at the 100K USDCx pre-audit cap once §1.5 launch gates clear (Class A overrides clear + axis One-Yellow + internal review complete). Expected TVL range $500-$25K; success measured by Phase-1 criteria (no CRITICAL exploits + working automation + community trust + audit-firm handoff readiness), not by hitting the cap. External audit timeline target Q2-Q3 2027 if §8.1 funding stack delivers; if it doesn't, V1 stays in this phase indefinitely on mainnet (100K cap — whitepaper §8.1 Option D).
- **Phase 2 user-facing (post-external-audit, conditional on §8.1 funding outcome; whitepaper Phase 3 → 4)**: cap lifted toward Stage 3 ($500K → $2M) per audit findings, target user expands toward $100-$10K hands-off depositors, governance rotation toward Phase 2+ signer slate.

V1 is designed for a specific profile in either phase: someone who holds USDCx, won't use a CEX, prefers not to run Liqwid manually, and accepts smart-contract risk in exchange for hands-off blended stablecoin yield. If that sounds like you, start small and read the [whitepaper](../whitepaper/whitepaper.md). If it doesn't, that's fine — we'd rather you find the product that actually fits your needs, whether that's Direct Liqwid, a CEX earn product, or simply continuing to self-custody USDCx.
...
- **V1's success threshold is $500K-$1M TVL, not $20M.** The 4.5% performance fee × that TVL × ~6% blended yield generates ~$270-540/year of keeper-share revenue at V1 launch's 40% keeper share, enough for the keeper to cover its own infra ops cost. That's the **tier (a)/(b) self-sustain** target. The widely-discussed "$20M TVL" figure is a separate **tier (c) audit-reserve self-funding** target (the protocol covering its own future external audit cycles from the treasury's 60% share at V1 launch), which is a Phase 3+ stretch goal, not a V1 prerequisite. Full three-tier breakdown in whitepaper §4.3.
- **Audit funding stack — volunteer-builder + community-funded model (whitepaper §0.2 + §8.1).** V1 launches on mainnet at the pre-audit 100K cap **regardless of audit funding status** — audit is a **cap-lift gate, not a launch gate**. Funding stack: (a) grant pool — Catalyst ($30-50K when a Round opens) + Cardano Foundation + Intersect Member Committee + Aiken Foundation, combined potential $30-150K depending on which sources deliver; **Catalyst is currently paused / restructuring with no confirmed Round resumption timeline**, so V1 does not depend on any single grant source; (b) audit-firm public-goods rate, 30-50% off the $50-150K base (multi-reviewer engagement model can push base toward $70-100K); (c) scope reduction from heritage internal audit (-$15-25K); (d) **founder gap-fill cap ~$15K** — founder commits up to ~$15K personal out-of-pocket to bridge small shortfalls between (a)/(b)/(c) and final audit price, and **explicitly does NOT commit to underwriting the full $50-150K audit cost in any scenario**. The $50-150K engaged-amount range in whitepaper §8.1 spans multi-reviewer / scope-reduced low-end (~$50K) to pre-discount single-firm full price (~$150K). **If the funding stack underperforms beyond $15K gap-fill capacity**, V1 follows the whitepaper §8.1 Options A-D contingency tree (audit timeline extension / scope-reduced audit / community DAO crowdfund / permanent pre-audit 100K cap) — all four options keep V1 live on mainnet at the 100K cap; they only determine the cap-lift pathway. **Audit-funding failure is NOT a sunset trigger** — whitepaper §0.2 + §4.1 + §8.1 are explicit on this. Under §8.1 Option D, V1 stays on mainnet at the 100K cap indefinitely; sunset only triggers under §1.5 Class A overrides (Liqwid / USDCx / Cardano chain / oracle incidents) or explicit founder sunset decision.
- **The 2 independent SPO governance signers are Cardano community-service roles, not paid positions.** They get a soul-bound recognition NFT and the option to take a 5-10% governance pool share in Phase 2+ (gated on $500K TVL milestone) — both are **upside if V1 succeeds, not primary motivation**. Selection criteria: ≥ 2 years mainnet SPO operating, public on-chain identity, no prior commercial relationship with the founder. **SPO recruitment is the launch target, not a launch blocker** — if recruitment lags by mainnet ceremony, V1 launches under the §5.5.1 three-layer governance safety design with the founder operating all 3 signer seats via key separation; depositor recovery paths (`emergency-withdraw`, hard caps, 7-day inactivity gate) do not depend on whether SPO recruitment has landed. Actual launch config is therefore either **3-of-3 with SPOs** or **founder-solo-with-key-separation under §5.5.1 fallback**. Whitepaper §5.5 + §5.5.1 + §7.4 cover this in full.
