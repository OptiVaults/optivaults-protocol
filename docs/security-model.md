...
**Historically closed variant (internal-verification-era):** a grief path involving `timelock = 0` plus 1-signer `CancelAction` was closed during internal review. Current `QueueAction` enforces `timelock_ms >= min_timelock_ms(action_kind)` (§3 of governance.md), so every queued action sits at ≥ its documented minimum — bounded by `ActionKind`, ranging from 0 (emergency) and 48h (slippage / per-market liveness toggles) up to 21 days (longest, `UpdateFeeSplit`), with `EmergencyWithdraw` / `FastUpdateMarkets` at the 0-1h floor.
