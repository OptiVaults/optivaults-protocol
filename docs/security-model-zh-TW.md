...
| VaultDatum Tiered Immutability | [`pattern-rationale-vault-datum-tiered-zh-TW.md`](../spec/pattern-rationale-vault-datum-tiered-zh-TW.md) | 「跨 non-deploy redeemer 的身份漂移」 — 透過在每條會產出 continuing output 的 redeemer 上呼叫 `check_immutable_fields` 強制 Tier 1 不可變性。「跨 keeper / governance / user redeemer 的授權混淆」 — Tier 2 變更必須走專屬治理 redeemer。審計面從 O(redeemer × 欄位)縮到 O(redeemer + tier) |
