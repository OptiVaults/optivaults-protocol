...
|--------|-----------------|--------|-----------|
| (a) Grant stack — Catalyst + Cardano Foundation + Intersect Member Committee + Aiken Foundation | $30K-$150K combined potential, $30K-$50K from Catalyst alone | **Catalyst paused / restructuring as of writing — no confirmed Round timeline.** Parallel outreach to Cardano Foundation / Intersect / Aiken Foundation reduces single-source dependency. V1 retains (a) as candidate pending resumption but does not depend on any single grant source. | Whitepaper §8.1 |
| (b) Audit-firm public-goods rate | 30-50% discount on $50K-$150K base; multi-reviewer engagement model can also push base toward $70K-$100K | Outreach pending | Whitepaper §8.1 |
| (c) Scope reduction via extensive internal audit history | -$15K-$25K from full scope | Heritage audit work | `audit-scope.md §1` |
| (d) Founder gap-fill (capped, NOT underwriter) | **Up to ~$15K personal out-of-pocket** to bridge small shortfalls; founder **explicitly does NOT commit to underwriting full audit cost in any scenario** — see whitepaper §0.2 + §4.1 volunteer-builder framing | Founding-capital allocation (capped) | Whitepaper §0.2 / §4.1 / §8.1 |

Audit timeline (Q2-Q3 2027 target — see `audit-scope.md §5`) is set explicitly later than the original Q3 2026 plan to accommodate (i) Catalyst Round resumption probability, (ii) parallel Cardano Foundation / Intersect / Aiken Foundation outreach, and (iii) the community / DAO contribution-pool path (whitepaper §8.1 Option C) maturing if (a)/(b) outcomes are conservative. Total expected founder out-of-pocket after stack: **$0-$15K** under any scenario, hard-capped at the gap-fill amount. If the funding stack underperforms beyond $15K shortfall, V1 follows the §8.1 Options A-D contingency tree (timeline extension / scope-reduced audit / community crowdfund / permanent Preprod under Apache 2.0) rather than escalating founder self-funding. This volunteer-builder model lets V1 pursue external audit completion without VC / token fundraising / equity-style dilution / or founder financial overcommitment. Status updates for each source published via GitHub issue labeled `audit-funding-status` as they progress.
...
- **V1 launches in a bootstrapping phase** — at 100K TVL, revenue does not fully cover operating costs. V1 has three self-sustain tiers (§5.3): (a) founder-keeper marginal ops break even at $185K–$555K TVL (current APY) / $555K–$1.67M (pessimistic APY); (b) non-founder professional keeper at $740K–$1.48M / $2.22M–$4.44M; (c) institutional ops + audit-reserve accrual at $20M+ / $50M+. V1's actual target is tier (a)/(b); tier (c) is deliberately NOT a target — audit funding comes from §5.1 funding stack (grants + public-goods rate + scope reduction + capped founder gap-fill ~$15K per §5.2.1 (d) — founder is NOT audit underwriter, see whitepaper §0.2), not from treasury accrual. Initial Phase 1 operating shortfalls are absorbed by founder personal-income subsidy under whitepaper §4.3.1 minimal-operations policy; if growth stalls below tier (a) AND external triggers (§1.5) or audit-funding failure (§8.1 Options A-D) materialise, §7.2 sunset protocol engages.
