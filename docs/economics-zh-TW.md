...
V1 的公共財定位(見白皮書 §8.1 + Executive Summary)支援一個非稀釋性的資金堆疊,覆蓋 2026 Q3 的外部審計,而不用涉及 VC / token / SAFE / SAFT:

| 來源 | 預期金額 | 狀態 | 出處 |
|------|---------|------|------|
| (a) Cardano Project Catalyst 撥款 | $30K-$50K | Round 15+(2026 Q2-Q3)提案 | 白皮書 §8.1 |
| (b) 審計事務所公共財費率 | 在 $100K-$150K 全價上折 30-50% | 接洽中 | 白皮書 §8.1 |
| (c) 藉大量 heritage 內部審計史縮減範圍 | 自全範圍省 $15K-$25K | Heritage 審計工作 | `audit-scope.md §1` |
| (d) 創辦人自付殘額 | $20K-$40K | 啟動資金分配 | 白皮書 §4.1 |

堆疊組合後,創辦人預期自付殘額 **$20K-$40K**,相較若全自費 $50K-$150K。這個堆疊讓 V1 能在**不 VC / 不發 token / 不稀釋 Apache 2.0 公共財姿態**的前提下完成外部審計。各來源狀態更新,以 `audit-funding-status` 標籤的 GitHub issue 公開發布。
...
| 1M | 540 | 180 | −180(虧) |
| 5M | 2,700 | 900 | +540(薄利) |
| 10M | 5,400 | 1,800 | +1,440(經濟上可行) |
| 100M | 54,000 | 18,000 | +17,640(具吸引力) |
...
在 pre-audit 的 100K TVL 上限下,OptiVaults 自己的營收並不覆蓋運營成本。差額約為**每年 USD 300–2,000**(視基礎設施選擇而定),由專案的**啟動資金**在 pre-audit / 初期存款階段承擔。這對新 DeFi 協議而言是正常的啟動狀態,預期隨著 TVL 進入自給區間後自然解決。

### 7.1 連續性姿態

V1 運營姿態:**「在 2026 Q3 審計窗口 + post-audit TVL 爬升期間維持運營,直到啟動資金用盡為止」**——**不是**一份可合約強制執行的長期保證。
...
在 2026 Q3 審計之後,100K USDCx TVL 上限預期分階段放寬。確切時程會跟審計報告一起發佈,這裡不事前承諾;目前意向是「把審計發現的修補」對應到每個上限放寬里程碑的分階段爬升。代表性的示意時程:
